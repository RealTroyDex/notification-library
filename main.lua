...
-- Define Instances and their properties
local Instances = {
...
-- Check if Notification GUI already exists
local plrgui = game.Players.LocalPlayer.PlayerGui
if not plrgui:FindFirstChild("Notification") then
	Instances.Notification.Name = "Notification"
	Instances.Notification.Parent = plrgui
	...
	Instances.Main_Holder.Position = UDim2.new(1.01, 0, 0.89, 0)
	...
	Instances.icon.Position = UDim2.new(0.019, 0, 0.1, 0)
...
function notification.notify(text, duration)
	if text and duration and not playing then
		...
		print("Text or duration not provided, or in cooldown.")
	end
end
...
function notification.error(text, duration)
	if text and duration and not playing then
		...
		error("Text or duration not provided, or in cooldown.")
	end
end
